...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        gaz_list,batch_word, batch_biword, batch_wordlen, batch_label, layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask, mask, _  = batchify_with_label(instance, data.HP_gpu, data.HP_num_layer, True)
        tag_seq, gaz_match = model(gaz_list,batch_word, batch_biword, batch_wordlen, layer_gaz, gaz_count,gaz_chars, gaz_mask, gazchar_mask, mask)
...
def batchify_with_label(input_batch_list, gpu, num_layer, volatile_flag=False):
    ...
    bin_label_tensor = autograd.Variable(torch.zeros(batch_size)).long()
    ...
    for b, (seq, biseq, label, seqlen, layergaz, gazmask, gazcount, gazchar, gazchar_mask, gaznum, gazlen) in enumerate(zip(words, biwords, labels, word_seq_lengths, layer_gazs, gaz_mask, gaz_count, gaz_chars, gazchar_mask, gaz_num, gaz_len)):
        ...
        if len(set(label))==1:
            bin_label_tensor[b] = 0
        else:
            bin_label_tensor[b] = 1
    ...
    if gpu:
        ...
        mask = mask.cuda()
        bin_label_tensor = bin_label_tensor.cuda()

    return gazs, word_seq_tensor, biword_seq_tensor, word_seq_lengths, label_seq_tensor, layer_gaz_tensor, gaz_count_tensor,gaz_chars_tensor, gaz_mask_tensor, gazchar_mask_tensor, mask, bin_label_tensor
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            gaz_list,  batch_word, batch_biword, batch_wordlen, batch_label, layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask, mask, batch_bin_label= batchify_with_label(instance, data.HP_gpu,data.HP_num_layer)

            instance_count += 1
            ner_loss, tag_seq, bin_pred = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen, layer_gaz, gaz_count,gaz_chars, gaz_mask, gazchar_mask, mask, batch_label)
            # _, _, bin_pred = model(gaz_list, batch_word, batch_biword, batch_wordlen, layer_gaz, gaz_count,gaz_chars, gaz_mask, gazchar_mask, mask)
            right, whole = predict_check(tag_seq, batch_label, mask)
            right_token += right
            whole_token += whole
            bin_loss = criterion(bin_pred, batch_bin_label)
            loss = ner_loss + 1.5*bin_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--status', choices=['train', 'test', 'decode','gaz'], help='update algorithm', default='train')
    ...
    char_parser = parser.add_mutually_exclusive_group(required=False)
    char_parser.add_argument('--use_char', dest='use_char', action='store_true')
    char_parser.add_argument('--no-use_char', dest='use_char', action='store_false')
    parser.set_defaults(use_char=False)

    count_parser = parser.add_mutually_exclusive_group(required=False)
    count_parser.add_argument('--use_count', dest='use_count', action='store_true')
    count_parser.add_argument('--no-use_count', dest='use_count', action='store_false')
    parser.set_defaults(use_count=True)
